...
void idLexer::Error( const char *str, ... ) {
	...
	if ( idLexer::flags & LEXFL_NOFATALERRORS ) {
		idReportWarning( "file %s, line %d: %s", idLexer::filename.c_str(), idLexer::line, text );
	} else {
		idReportError( "file %s, line %d: %s", idLexer::filename.c_str(), idLexer::line, text );
	}
}
...
int idLexer::ReadToken( idToken *token ) {
	...
	if ( !loaded ) {
		idReportError( "idLexer::ReadToken: no file loaded" );
		return 0;
	}
...
void idLexer::UnreadToken( const idToken *token ) {
	if ( idLexer::tokenavailable ) {
		idReportError( "idLexer::unreadToken, unread token twice\n" );
	}
	idLexer::token = *token;
...
int idLexer::LoadFile( const char *filename, bool OSPath ) {
	...
	if ( idLexer::loaded ) {
		idReportError("idLexer::LoadFile: another script already loaded");
		return false;
	}
...
int idLexer::LoadMemory( const char *ptr, int length, const char *name, int startLine ) {
	if ( idLexer::loaded ) {
		idReportError("idLexer::LoadMemory: another script already loaded");
		return false;
	}
