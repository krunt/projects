...
public:
   ...
   void           GetAxis(idVec3 &forward, idVec3 &left, idVec3 &up) const;
   void           GetTranslation(idVec3 &translation) const;

   void           SetAxis(const idVec3 &forward);
   void           SetTranslation(const idVec3 &translation);
...
#define MATX_MAX_TEMP		1024*1024
...
{
public:
   idSparseMatX(int rows, int cols) : m_rows(rows), m_cols(cols)
   {
      m_row_bitmap.resize((m_rows+7)/8, 0);
   }
   ...
   {
      ReprType::key_type key= std::make_pair(row,col);
      ReprType::const_iterator it= m_repr.find(key);
      return it!=m_repr.end()?(*it).second:0.0f;
   }

   float &InsertGet(unsigned int row, unsigned int col)
   {
      ReprType::key_type key= std::make_pair(row,col);
      /*
      ReprType::iterator it= m_repr.find(key);
      if (it!=m_repr.end())
      {
         return (*it).second;
      }
      */
      m_row_bitmap[row/8]|= (1<<(row&7));
      return m_repr[key];
      //return m_repr[key]= 0.0f;
   }

   void SetDimensions(int nrows, int ncols)
   {
      assert(nrows<=m_rows&&ncols<=m_cols);
      m_rows= nrows;
      m_cols= ncols;
   }
   ...
   idVecX operator*(const idVecX &vec) const
   {
      assert(vec.GetDimension()==m_cols);
      
      idVecX res(m_rows);
      res.Zero();

      if (!m_optimized_repr.empty())
      {
         for (int i= 0; i<m_optimized_repr.size(); ++i)
         {
            int row_index= m_optimized_repr[i].first;

            float result= 0.0f;

            for (int j=0;j<m_optimized_repr[i].second.size(); ++j)
            {
               auto &v= m_optimized_repr[i].second[j];
               int column_index= v.first;

               assert(column_index<m_cols);
               result+= v.second*vec[column_index];
            }
      
            res[row_index]= result;
         }
      }
      else
      {
         assert(m_row_bitmap.size()*8>=m_rows);

         for (int row_index= 0; row_index<m_rows; ++row_index)
         {
            if (!m_row_bitmap[row_index/8])
            {
               row_index+=8-1;
               continue;
            }

            if (!(m_row_bitmap[row_index/8]&(1<<(row_index&7))))
            {
               continue;
            }

            float result= 0.0f;
            for (ReprType::const_iterator it= m_repr.lower_bound(std::make_pair(row_index, 0)),
               end_it= m_repr.upper_bound(std::make_pair(row_index, m_cols-1)); it != end_it; ++it)
            {
               unsigned int column= (*it).first.second;
               float value= (*it).second;

               assert(column<vec.GetDimension());

               result+= value*vec[column];
            }
   ...
   // $TODO optimize
   idSparseMatX SquareSubMatrix(const idSparseMatX &other, int dimension)
   {
      assert(dimension<=m_rows&&dimension<=m_cols);

      idSparseMatX res= other;

      res.m_rows= dimension;
      res.m_cols= dimension;
   ...
   {
   public:
      virtual void Solve(const idSparseMatX &p, const idVecX &b, idVecX &x) const
      {
         for (int k=0; k<b.GetDimension(); ++k)
         {
            float diag= p.Get(k,k);
   ...
   static void ConjugateGradient(const idSparseMatX &S, const idVecX &b, idVecX &x, int iter, float epsilon)
   {
      CPreconditioner preconditioner;
      idVecX r(x.GetDimension()), d(x.GetDimension()), q(x.GetDimension()), s(x.GetDimension());
      float sigma_old, sigma_new, alpha, beta;

      r= b-S*x;

      preconditioner.Solve(S, r, d);

      sigma_new= r*d;

      for (int i=0;i<iter&&epsilon<sigma_new; ++i)
      {
         q= S*d;
         alpha= sigma_new / (d*q);
   ...
   void OptimizeRepr()
   {
      ...
      for (int row_index= 0; row_index<m_rows; ++row_index)
      {
         ...
         m_optimized_repr.push_back(std::make_pair(row_index, std::vector<std::pair<int, float> >()));
         std::pair<int, std::vector<std::pair<int, float> > > &repr= m_optimized_repr.back();
            ...
            end_it= m_repr.upper_bound(std::make_pair(row_index, m_cols-1)); it != end_it; ++it)
         {
            ...
            repr.second.push_back(std::make_pair(column, value));
         }
      }
   }
...
private:
   int m_rows, m_cols;
   typedef std::map<std::pair<unsigned int, unsigned int>, float> ReprType;
   ReprType m_repr;
   std::vector<int> m_row_bitmap; 

   // optimized structure
   std::vector<std::pair<int, std::vector<std::pair<int, float> > > > m_optimized_repr;
};
...
ID_INLINE idMatX &idMatX::SwapRows( int r1, int r2 ) {
	...
	ptr = (float *) Mem_Alloc16( numColumns * sizeof( float ) );
