...
#define VERBOSE( x ) { if ( ase.verbose ) {  } }
...
static void ASE_KeyMAP_DIFFUSE( const char *token )
{
	...
	if ( !strcmp( token, "*BITMAP" ) )
	{
		...
		strncpy( ase.currentMaterial->name, matname.c_str(), sizeof( ase.currentMaterial->name ) );
	}
...
static void ASE_KeyMESH_VERTEX_LIST( const char *token )
{
	...
	if ( !strcmp( token, "*MESH_VERTEX" ) )
	{
		...
		{
			idReportError( "ase.currentVertex >= pMesh->numVertexes" );
		}
	}
	else
	{
		idReportError( "Unknown token '%s' while parsing MESH_VERTEX_LIST", token );
	}
}
...
/*
		...
		{
			idReportError( "No *MESH_MTLID found for face!" );
		}
*/
	...
	{
		idReportError( "Unknown token '%s' while parsing MESH_FACE_LIST", token );
	}
}
...
static void ASE_KeyTFACE_LIST( const char *token )
{
	...
	{
		idReportError( "Unknown token '%s' in MESH_TFACE", token );
	}
}
...
static void ASE_KeyCFACE_LIST( const char *token )
{
	...
	{
		idReportError( "Unknown token '%s' in MESH_CFACE", token );
	}
}
...
static void ASE_KeyMESH_TVERTLIST( const char *token )
{
	...
	if ( !strcmp( token, "*MESH_TVERT" ) )
	{
		...
		{
			idReportError( "ase.currentVertex > pMesh->numTVertexes" );
		}
	}
	else
	{
		idReportError( "Unknown token '%s' while parsing MESH_TVERTLIST", token );
	}
}
...
static void ASE_KeyMESH_CVERTLIST( const char *token )
{
	...
	if ( !strcmp( token, "*MESH_VERTCOL" ) )
	{
		...
		{
			idReportError( "ase.currentVertex > pMesh->numCVertexes" );
		}
	}
	else {
		idReportError( "Unknown token '%s' while parsing MESH_CVERTLIST", token );
	}
}
...
static void ASE_KeyMESH_NORMALS( const char *token )
{
	...
	if ( !strcmp( token, "*MESH_FACENORMAL" ) )
	{
		...
		if ( num >= pMesh->numFaces || num < 0 ) {
			idReportError( "MESH_NORMALS face index out of range: %i", num );
		}

		if ( num != ase.currentFace ) {
			idReportError( "MESH_NORMALS face index != currentFace" );
		}
	...
	else if ( !strcmp( token, "*MESH_VERTEXNORMAL" ) )
	{
		...
		if ( num >= pMesh->numVertexes || num < 0 ) {
			idReportError( "MESH_NORMALS vertex index out of range: %i", num );
		}
		...
		if ( v >= 3 ) {
			idReportError( "MESH_NORMALS vertex index doesn't match face" );
			return;
		}
...
static void ASE_KeyMESH( const char *token )
{
	...
	else if ( !strcmp( token, "*MESH_NUMTVFACES" ) )
	{
		...
		{
			idReportError( "MESH_NUMTVFACES != MESH_NUMFACES" );
		}
	}
	else if ( !strcmp( token, "*MESH_NUMCVFACES" ) )
	{
		...
		if ( pMesh->numTVFaces != pMesh->numFaces )
		{
			idReportError( "MESH_NUMCVFACES != MESH_NUMFACES" );
		}
	}
	...
	{
		if ( !pMesh->faces ) {
			idReportError( "*MESH_TFACELIST before *MESH_FACE_LIST" );
		}
		ase.currentFace = 0;
	...
	{
		if ( !pMesh->faces ) {
			idReportError( "*MESH_CFACELIST before *MESH_FACE_LIST" );
		}
		ase.currentFace = 0;
	...
	{
		if ( !pMesh->faces ) {
			VERBOSE( "*MESH_NORMALS before *MESH_FACE_LIST" );
		}
		ase.currentFace = 0;
...
static void ASE_KeyMESH_ANIMATION( const char *token )
{
	...
	{
		idReportError( "Unknown token '%s' while parsing MESH_ANIMATION", token );
	}
}
...
static void ASE_KeyGEOMOBJECT( const char *token )
{
	...
	if ( !strcmp( token, "*NODE_NAME" ) )
	{
		ASE_GetToken( true );
		VERBOSE( ( " %s\n", ase.token ) );
		strncpy( object->name, ase.token, sizeof( object->name ) );
	}
...
aseModel_t *ASE_Load( const char *fileName ) {
   ...
   buf= (char*)idBloatFile(fileName, NULL);
   if (!buf) {
      return NULL;
   }
