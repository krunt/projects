...
bool idBounds::IntersectRay(const idVec3 &start, const idVec3 &dir, float scale, float *hitt0,
   float *hitt1) const {
      float t0 = 0, t1 = scale;
      for (int i = 0; i < 3; ++i) {
         // Update interval for _i_th bounding box slab
         float invRayDir = 1.f / dir[i];
         float tNear = (GetMin()[i] - start[i]) * invRayDir;
         float tFar  = (GetMax()[i] - start[i]) * invRayDir;
...
void idBounds::FromPoints( const idVec3 *points, const int numPoints ) {
   b[0] = idVec3(idMath::INFINITY); 
   b[1] = idVec3(-idMath::INFINITY);

   for (int i=0; i<numPoints; ++i)
   {
      b[0][0]= Min(b[0][0],points[i][0]);
      b[0][1]= Min(b[0][1],points[i][1]);
      b[0][2]= Min(b[0][2],points[i][2]);
   }

   for (int i=0; i<numPoints; ++i)
   {
      b[1][0]= Max(b[1][0],points[i][0]);
      b[1][1]= Max(b[1][1],points[i][1]);
      b[1][2]= Max(b[1][2],points[i][2]);
   }
}
...
float idBounds::SurfaceArea() const
{
   idVec3 d = GetMax() - GetMin();
   return 2.f * (d.x * d.y + d.x * d.z + d.y * d.z);
}

int         idBounds::GetMaximumExtentIndex() const
{
   float maxExtent= 0.0f;
   int extentIndex= 0;
   for (int i=0; i<3; ++i)
   {
      float extent= GetMax()[i]-GetMin()[i];
      if (maxExtent<extent)
      {
         maxExtent= extent;
         extentIndex= i;
