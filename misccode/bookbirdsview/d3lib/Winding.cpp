...
int idWinding::Split( const idPlane &plane, const float epsilon, idWinding **front, idWinding **back ) const {
	...
	*back = b = new idWinding(maxpts);
	...
	if ( f->numPoints > maxpts || b->numPoints > maxpts ) {
		idFatalError( "idWinding::Split: points exceeded estimate." );
	}
...
bool idWinding::Check( bool print ) const {
	...
	if ( numPoints < 3 ) {
		if ( print ) {
			idReportWarning( "idWinding::Check: only %i points.", numPoints );
		}
		return false;
	}
	...
	if ( area < 1.0f ) {
		if ( print ) {
			idReportWarning( "idWinding::Check: tiny area: %f", area );
		}
		return false;
	}
	...
	for ( i = 0; i < numPoints; i++ ) {
		...
		for ( j = 0; j < 3; j++ ) {
			if ( p1[j] >= MAX_WORLD_COORD || p1[j] <= MIN_WORLD_COORD ) {
				if ( print ) {
					idReportWarning( "idWinding::Check: point %d outside world %c-axis: %f", i, 'X'+j, p1[j] );
				}
				return false;
			}
		}
		...
		if ( d < -ON_EPSILON || d > ON_EPSILON ) {
			if ( print ) {
				idReportWarning( "idWinding::Check: point %d off plane.", i );
			}
			return false;
		}
		...
		if ( dir.Length() < ON_EPSILON) {
			if ( print ) {
				idReportWarning( "idWinding::Check: edge %d is degenerate.", i );
			}
			return false;
		}
		...
		for ( j = 0; j < numPoints; j++ ) {
			...
			if ( d > edgedist ) {
				if ( print ) {
					idReportWarning( "idWinding::Check: non-convex." );
				}
				return false;
			}
		}
...
void idWinding::RemovePoint( int point ) {
	if ( point < 0 || point >= numPoints ) {
		idFatalError( "idWinding::removePoint: point out of range" );
	}
...
void idWinding::InsertPoint( const idVec5 &point, int spot ) {
	...
	if ( spot > numPoints ) {
		idFatalError( "idWinding::insertPoint: spot > numPoints" );
	}

	if ( spot < 0 ) {
		idFatalError( "idWinding::insertPoint: spot < 0" );
	}
...
void idWinding::Print() const {
	...
	for ( i = 0; i < numPoints; i++ ) {
		idReportWarning( "(%5.1f, %5.1f, %5.1f)\n", p[i][0], p[i][1], p[i][2] );
	}
}
...
bool idFixedWinding::ReAllocate( int n, bool keep ) {
	...
	if ( n > MAX_POINTS_ON_WINDING ) {
		idReportWarning("WARNING: idFixedWinding -> MAX_POINTS_ON_WINDING overflowed\n");
		return false;
	}
	return true;
}
