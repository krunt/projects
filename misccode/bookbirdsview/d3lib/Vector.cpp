...
void idMinMax(const idVec3 *points, int numPoints, idVec3 &vmin, idVec3 &vmax)
{
   vmin = idVec3(idMath::INFINITY); 
   vmax = idVec3(-idMath::INFINITY);

   for (int i=0; i<numPoints; ++i)
   {
      vmin[0]= Min(vmin[0],points[i][0]);
      vmin[1]= Min(vmin[1],points[i][1]);
      vmin[2]= Min(vmin[2],points[i][2]);
   }

   for (int i=0; i<numPoints; ++i)
   {
      vmax[0]= Max(vmax[0],points[i][0]);
      vmax[1]= Max(vmax[1],points[i][1]);
      vmax[2]= Max(vmax[2],points[i][2]);
   }
}
