...
static void my_vio_delete(Vio* vio)
{
  if (!vio)
...
static int my_vio_errno(Vio *vio) {
    return 0;
}

static int my_vio_close(Vio * vio)
{
  int r=0;
...
static size_t my_vio_read(Vio * vio, uchar* buf, size_t size)
{
  size_t r;
  errno=0;					/* For linux */
...
static size_t my_vio_write(Vio * vio, const uchar* buf, size_t size)
{
  size_t r;
...
#ifdef DEBUG
  fprintf(stderr, "ssl_read: %d/%d/%d\n", 
          r, size, SSL_get_error((SSL*)vio->ssl_arg, r));
  if (SSL_get_error((SSL*)vio->ssl_arg, r)) {
...
#ifdef DEBUG
  fprintf(stderr, "ssl_write: %d/%d/%d\n", 
          r, size, SSL_get_error((SSL*)vio->ssl_arg, r));
#endif
  return r;
}
                      ...
                      const char *cipher)
{
  ...
  */
  ...
  SSL_CTX_set_mode(ssl_fd->ssl_context, SSL_MODE_ENABLE_PARTIAL_WRITE
		     ...
		     const char *cipher)
{
  ...
  //SSL_CTX_set_verify(ssl_fd->ssl_context, verify, NULL);
  SSL_CTX_set_verify(ssl_fd->ssl_context, SSL_VERIFY_NONE, NULL);
...
#endif /* HAVE_OPENSSL */
  vio->viodelete	=my_vio_delete;
  vio->vioerrno	=my_vio_errno;
  vio->read= my_vio_read;
  vio->write		=my_vio_write;
  vio->vioclose	=my_vio_close;
}
