...
void MyRender::Init( const playerView_t &view ) {
    ...
    m_screenMaterial = glMaterialCache.Get( "postprocess" );
}
...
void MyRender::Render( const playerView_t &view ) {
    ...
    for ( i = 0; i < m_surfs.size(); ++i ) {
        if ( m_surfs[i].m_surf.m_material == m_screenMaterial ) {
            continue;
        }
    ...
    m_offScreenTarget.CopyToTexture( 
            *static_cast<GLTexture *>( 
            static_cast<GLMaterial *>( m_screenMaterial.get() )->GetTexture().get() ) );
