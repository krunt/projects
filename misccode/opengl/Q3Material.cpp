...
static bool ParseVector( const char **text, int count, idVec3 *v ) {
	char	*token;
	int		i;
...
static int NameToSrcBlendMode( const char *name )
{
	...
	{
		return GLS_BLEND_ONE;
	}
	else if ( !Q_stricmp( name, "GL_ZERO" ) )
	{
		return GLS_BLEND_ZERO;
	}
	else if ( !Q_stricmp( name, "GL_DST_COLOR" ) )
	{
		return GLS_BLEND_DST_COLOR;
	}
	else if ( !Q_stricmp( name, "GL_ONE_MINUS_DST_COLOR" ) )
	{
		return GLS_BLEND_ONE_MINUS_DST_COLOR;
	}
	else if ( !Q_stricmp( name, "GL_SRC_ALPHA" ) )
	{
		return GLS_BLEND_SRC_ALPHA;
	}
	else if ( !Q_stricmp( name, "GL_ONE_MINUS_SRC_ALPHA" ) )
	{
		return GLS_BLEND_ONE_MINUS_SRC_ALPHA;
	}
	else if ( !Q_stricmp( name, "GL_DST_ALPHA" ) )
	{
		return GLS_BLEND_DST_ALPHA;
	}
	else if ( !Q_stricmp( name, "GL_ONE_MINUS_DST_ALPHA" ) )
	{
		return GLS_BLEND_ONE_MINUS_DST_ALPHA;
    }
	else if ( !Q_stricmp( name, "GL_SRC_COLOR" ) )
	{
		return GLS_BLEND_SRC_COLOR;
	}
	else if ( !Q_stricmp( name, "GL_ONE_MINUS_SRC_COLOR" ) )
	{
		return GLS_BLEND_ONE_MINUS_SRC_COLOR;
    }
...
{
    return NameToSrcBlendMode( name );
}
...
static bool ParseStage( shaderStage_t *stage, const char **text )
{
	...
	while ( 1 )
	{
		...
		if ( !token[0] )
		{
			msg_warning0( "WARNING: no matching '}' found\n" );
			return qfalse;
		}
		...
		else if ( !Q_stricmp( token, "blendfunc" ) )
		{
			...
			if ( !Q_stricmp( token, "add" ) ) {
				blendSrcBits = GLS_BLEND_ONE;
				blendDstBits = GLS_BLEND_ONE;
			} else if ( !Q_stricmp( token, "filter" ) ) {
				blendSrcBits = GLS_BLEND_DST_COLOR;
				blendDstBits = GLS_BLEND_ZERO;
			} else if ( !Q_stricmp( token, "blend" ) ) {
				blendSrcBits = GLS_BLEND_SRC_ALPHA;
				blendDstBits = GLS_BLEND_ONE_MINUS_SRC_ALPHA;
			} else {
...
bool Q3Material::ParseShader( const std::string &name, const std::string &contents ) {
	...
	while ( 1 )
	{
		...
		else if ( token[0] == '{' )
		{
            ...
            m_shader.m_stages.push_back( stage );
			continue;
		}
...
static void ParseShaderFile( const std::string &filename, std::map<std::string, std::string> &mp )
{
	char *token;
    const char *b, *p;
    std::string contents, sname, sbody;
    ...
    if ( contents.empty() ) {
        msg_failure( "shader `%s' filename is not found", filename.c_str() );
    }
...
void Q3Material::ScanShaders() {
    m_shadersTextMap.clear();

    fs::path p( "q3shaders" );

    for ( fs::directory_iterator b = fs::directory_iterator(p); 
            b != fs::directory_iterator(); ++b ) {
        if ( EndsWith( b->path().c_str(), ".shader" ) ) {
            ParseShaderFile( b->path().c_str(), m_shadersTextMap );
        }
    }
}
...
bool Q3Material::Init( const std::string &name ) {
    ...
    m_program = glProgramCache.Get( "q3shader" );
...
#define DEBUG
    ...
    const char *names[N] = {
        /*
        "stages[0].m_rgbWave.m_type",
        "stages[0].m_rgbGen",
        "stages[0].m_alphaWave.m_type",
        "stages[0].m_constantColor",
        "stages[0].m_texBundle.m_texture",
        "stages[0].m_glBlend",
        "stages[0].m_texBundle.m_numTexMods",
        "stages[0].m_texBundle.m_texMods[0].m_rotateSpeed",
        "stages[0].m_texBundle.m_texMods[1].m_texModType",
        "stages[0].m_texBundle.m_texMods[0].m_rotate",
        */

        "stages[0].m_texBundle.m_numTexMods",
        "stages[0].m_texBundle.m_texMods[0].m_texModType",
        "stages[0].m_texBundle.m_texMods[1].m_texModType",
        "stages[0].m_texBundle.m_texMods[2].m_texModType",
        "stages[0].m_glBlend",

        "stages[0].m_rgbWave.m_type",
        "stages[1].m_rgbWave.m_type",
        "stages[2].m_rgbWave.m_type",
    };
    ...
    _CH(glGetUniformIndices( m_program->m_program, N, names, indices ));
    _CH(glGetActiveUniformsiv( m_program->m_program, N, indices, GL_UNIFORM_OFFSET, offset ));
    _CH(glGetActiveUniformsiv( m_program->m_program, N, indices, GL_UNIFORM_SIZE, size ));

    for (int i = 0; i < N; ++i) {
        fprintf( stderr, "%s: size=%d offset=%d\n", names[i], size[i], offset[i] );
    }
...
int Q3Material::GetAnimationIndexFromBundle( const textureBundle_t &bundle, float bTime ) const {
    return 0;
}
...
void Q3Material::BindTextures( const CommonMaterialParams &params ) {
    for ( int i = 0; i < m_shader.m_stages.size(); ++i ) {
        ...
        boost::shared_ptr<TextureBase> texPtr = glTextureCache.Get( bundle.image[bindIndex] );
...
struct Std140OpenglAlignmentHelper::AHelper<waveForm_t, 0> 
{
    ...
    static const int sizeUnaligned
        = Std140OpenglAlignmentHelper::AHelper<GLint>::size
            + 4 * Std140OpenglAlignmentHelper::AHelper<float>::size;

    static const int size = sizeUnaligned;
};
...
struct Std140OpenglAlignmentHelper::AHelper<texModInfo_t, 0> 
{
    ...
    static const int sizeUnaligned
        = Std140OpenglAlignmentHelper::AHelper<GLint>::size
            + Std140OpenglAlignmentHelper::AHelper<waveForm_t>::size
            + Std140OpenglAlignmentHelper::AHelper<idMat2>::size
            + 3 * Std140OpenglAlignmentHelper::AHelper<idVec2>::size
            + Std140OpenglAlignmentHelper::AHelper<float>::size;
...
struct Std140OpenglAlignmentHelper::AHelper<textureBundle_t, 0> 
{
    ...
    static const int sizeUnaligned
        = 3 * Std140OpenglAlignmentHelper::AHelper<GLint>::size
        + Std140OpenglAlignmentHelper::AHelper<texModInfo_t, 4>::size;
...
struct Std140OpenglAlignmentHelper::AHelper<shaderStage_t, 0> 
{
    ...
    static const int sizeUnaligned
        ...
        + Std140OpenglAlignmentHelper::AHelper<waveForm_t>::size
        + Std140OpenglAlignmentHelper::AHelper<GLint>::size
        + Std140OpenglAlignmentHelper::AHelper<idVec4>::size
        + Std140OpenglAlignmentHelper::AHelper<textureBundle_t>::size
        + Std140OpenglAlignmentHelper::AHelper<GLint>::size;
...
{
    byte *p = pBuffer;
    int alignValue = Std140OpenglAlignmentHelper::GetAlignment<waveForm_t>::value;
...
byte *Q3StagesBlock::PackInternal( byte *pBuffer, 
        const texModInfo_t &texMod ) const
{
    byte *p = pBuffer;
    int alignValue = Std140OpenglAlignmentHelper::GetAlignment<texModInfo_t>::value;
...
byte *Q3StagesBlock::PackInternal( byte *pBuffer, 
        const textureBundle_t &texBundle ) const
{
    byte *p = pBuffer, *pp;
    int alignValue 
        = Std140OpenglAlignmentHelper::GetAlignment<textureBundle_t>::value;
    ...
    p = AlignUp( p, 
        Std140OpenglAlignmentHelper::GetAlignment<texModInfo_t, 4>::value );

    pp = p + Std140OpenglAlignmentHelper::GetSize<texModInfo_t, 3>::value
            + Std140OpenglAlignmentHelper::GetSize<texModInfo_t, 0>::value;

    for ( int i = 0; i < texBundle.numTexMods; ++i ) {
        p = PackInternal( p, texBundle.texMods[i] );
        if ( i + 1 != 4 ) {
            p = AlignUp( p, Std140OpenglAlignmentHelper::GetAlignment<texModInfo_t, 4>::value );
        }
    }
        ...
        const shaderStage_t &stage ) const
{
    byte *p = pBuffer;
    int alignValue 
        = Std140OpenglAlignmentHelper::GetAlignment<shaderStage_t>::value;
    ...
    p = Std140GLSLPacker::Pack( p, stage.constantColor );
