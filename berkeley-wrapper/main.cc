...
public:
    FieldMerger(field_predicatelist_type &lhs, field_predicatelist_type &rhs)
        : lhs_(lhs), rhs_(rhs)
    {}

    void merge() {
        typedef typename mpl::at<IndexFields, mpl::int_<index> >::type EltType;
        typedef typename Argument<index, IndexFields, EltType>::this_type ArgType;

        if (rhs_[index].get_max()) {
            ArgType().replace_max_if_needed(
                lhs_[index], *static_cast<const EltType*>(rhs_[index].get_max()));
        }
        if (rhs_[index].get_min()) {
            ArgType().replace_min_if_needed(
                lhs_[index], *static_cast<const EltType*>(rhs_[index].get_min()));
        }

        FieldMerger<index - 1, IndexFields>(lhs_, rhs_).merge();
    }

private:
    field_predicatelist_type &lhs_;
    field_predicatelist_type &rhs_;
};

template <typename IndexFields>
class FieldMerger<-1, IndexFields> {
public:
    typedef typename PredicateEntity<IndexFields>::field_predicatelist_type 
        field_predicatelist_type;
...
public:
    ...
    this_type &operator&&(Argument<index2, IndexFields, T> &argument) {
        field_predicatelist_type &other_predicates = argument.field_predicates();
        merge_them(other_predicates);
        return *this;
    }

    this_type &operator<(std::string val) {
        replace_max_if_needed(this->field_predicates()[index], 
                String(const_cast<char*>(val.data()), val.size()));
        return *this;
    }

    this_type &operator>(std::string val) {
        replace_min_if_needed(this->field_predicates()[index], 
                String(const_cast<char*>(val.data()), val.size()));
        return *this;

    }

    void merge_them(field_predicatelist_type &rhs) {
        FieldMerger<mpl::size<IndexFields>::value - 1, IndexFields>(
                    this->field_predicates(), rhs).merge();
    }

    void replace_max_if_needed(MinMaxFieldPredicate &predicate, const String &val) {
    ...
    void replace_min_if_needed(MinMaxFieldPredicate &predicate, const String &val) {
...
public:
    ...
    this_type &operator<(int val) {
        replace_max_if_needed(this->field_predicates()[index], Int32(val));
        return *this;
    }

    this_type &operator>(int val) {
        replace_min_if_needed(this->field_predicates()[index], Int32(val));
        return *this;
    }
    ...
    void replace_min_if_needed(MinMaxFieldPredicate &predicate, const Int32 &val) {
        if (predicate.get_max() 
                && predicate.get_max()->compare(val) > 0)
        {
            predicate.get_max()->dispose();
    ...
    void replace_max_if_needed(MinMaxFieldPredicate &predicate, const Int32 &val) {
        if (predicate.get_min() 
                && predicate.get_min()->compare(val) < 0) 
        {
            predicate.get_min()->dispose();
...
void bulkinsert(Table<IndexFields> &table) {
    static const char *randarray[10] = {
        "access-log", "r-log", "b-log", "smile-log", "mil-log",
        "bim-log", "freak-log", "w-log", "z-log", "x-log"
    };
    for (int i = 0; i < 100000; ++i) {
        const char *f1 = randarray[rand() % 10];
        const char *f4 = randarray[rand() % 10];
        const char *value = randarray[rand() % 10];
        table.insert(mkstring(f1), Int32(rand() % 10000),
            Int32(rand() % 10000), mkstring(f4), mkstring(value));
    }
}
...
int main() {
    ...
    */
    ...
    /* std::cout << predicate->to_string() << std::endl; */
